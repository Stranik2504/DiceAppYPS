        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                while (input != "exit")
                {
                    ...
                    if (!commands.Contains("unvisible")) { if (commands.Contains("green")) { Print(input, ConsoleColor.Green); } else if (commands.Contains("blue")) { Print(input, ConsoleColor.Blue); } else { Console.WriteLine(input); } }
                    if (commands.Contains("get-value")) { string text = ""; try { text = Console.ReadLine(); } catch { } manager.SendMessage(text == "" || text == null || text == " " ? "ok" : text); }
